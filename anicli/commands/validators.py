...
def enumerate_completer(
    results: List[Any], *, whitelist_commands: Optional[Dict[str, str]] = None
) -> WordCompleter:
...
def _number_func(
    digit: str, max_len: int, whitelist_commands: Optional[Set[str]] = None
):
    whitelist_commands = whitelist_commands or set()
    return digit.isdigit() and 0 <= int(digit) < max_len or digit in whitelist_commands


def number_validator(
    max_len: Union[int, list], whitelist_commands: Optional[Set[str]] = None
) -> Validator:
...
def _is_num_slice_or_int(digit: str, max_len: int, whitelist_commands: Set[str] = None):
    whitelist_commands = whitelist_commands or set()
    return (
...
def number_slice_validator(
    ...
    if isinstance(max_len, list):
        max_len = len(max_len)
    func = partial(
        _is_num_slice_or_int, max_len=max_len, whitelist_commands=whitelist_commands
    )
    if whitelist_commands:
