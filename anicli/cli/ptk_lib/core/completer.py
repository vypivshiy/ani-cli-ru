...
class NestedCompleterWithMeta(Completer):
    ...
    This completer handles nested dictionary structures for command completion,
    with support for dynamic completions and meta information.

    Args:
        options: Dictionary containing completion options with optional _meta keys. Has magic keys:
            - "_meta" - add meta description for item
            - "_completions" - pass extra competions for key
            - both keys can be combined
    ...
    def _resolve_dynamic(self, node):
        ...
        Args:
            node: The node to check for dynamic completions

        Returns:
            Dictionary of dynamic completions, or None if not found
        """
    ...
    def get_completions(
        ...
        """Generate completions based on the current document text.

        Args:
            document: The document containing the current input
            complete_event: Event object from prompt_toolkit (not used)

        Yields:
            Completion objects matching the current input
        """
    ...
    def from_nested_dict(cls, options: Dict[str, Any]) -> "NestedCompleterWithMeta":
        """Create a NestedCompleterWithMeta from a nested dictionary.

        Args:
            options: Dictionary containing completion options

        Returns:
            NestedCompleterWithMeta instance
        """
        return cls(options)
...
class SubCommandCompleter(Completer):
    ...
    Args:
        route: The CommandRoute containing sub-commands to complete
    """
...
class ContextualCompleter(Completer):
    ...
    This completer adapts its suggestions based on the current application state,
    showing FSM-specific completions when an FSM is active, or global command
    completions when in the main application.
    ...
    def get_completions(self, document: Document, complete_event) -> Iterable[Completion]:
        """Generate context-aware completions based on the current application state.
        ...
        Yields:
            Completion objects matching the current input and context
        """
...
class HybridFSMCompleter(Completer):
    ...
    This completer merges completions from both static sources (defined via decorators)
    and dynamic sources (generated by the active FSM), ensuring users get comprehensive
    suggestions regardless of the source.

    Args:
        app: Reference to the main Application instance
        static_completer: Optional static completer to combine with dynamic completions
    """
    ...
    def get_completions(self, document: Document, complete_event) -> Iterable[Completion]:
        """Generate combined completions from static and dynamic sources.
        ...
        Yields:
            Completion objects from both static and dynamic sources
        """
        current_fsm = self.app.current_fsm
